...
export async function executeCreateTask(action: ActionDefinition): Promise<void> {
  const config = action.config;

  const title = config.title as string;
  const datetime = config.datetime as string;
  const assignee = (config.assignee as string) || "PROCESS_OWNER";
  ...
  try {
    ...
    switch (assignee) {
      case "PROCESS_OWNER":
        ...
        break;
      case "CUSTOM":
        assigneeEmail = config.email as string;
        if (!assigneeEmail) {
